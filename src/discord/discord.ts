...
const voiceChannels = {
	name: 'Кибер комната',
	duo: '865697645920911371',
	trio: '865697670852378684',
...
export class Discord {
	...
	constructor(connection: Connection) {
		this.database = new DiscordDatabase(connection);
	}
	...
	private async login() {
		...
		this.client = client;
		this.guild = client.guilds.cache.find((guild) => guild.id === '437601028662231040');
		this.run();
	}

	/**
	 * Delete message
	 * @param time in seconds. Default: 10
	 */
	private _delete(msg: ds.Message, time?: number) {
		if (!time) time = 10;
		setTimeout(async () => {
			if (msg.deletable) {
				await msg.delete();
	 ...
	 */
	private async _log(title?: string, text?: string) {
	...
	private async isMod(userID: string): Promise<boolean> {
		const userMod = await this.guild.members.fetch(userID).then((user) => {
	...
	private async setBasicRole(): Promise<void> {
		...
		const members = (await this.guild.members.fetch()).toJSON();

		for (let i in members) {
			const member = members[i];

			if (!member.user.bot) {
				const databaseMember = await this.database.findOneByUserID(member.id);
				if (databaseMember == null) {
					...
					user.bans = 0;
					user.messages = 0;
					await this.database.insertOne(user);
					if (member.roles.cache.has('918778640869773334') === false) {
						const roleAdd = this.guild.roles.cache.find(
							(role, key, collection) => role.id === '918778640869773334'
						);
						await member.roles.add(roleAdd);
						this.logger.info(`${member.id} (${member.displayName}) не наш`);
					}
					...
					if (member.roles.cache.has('918626848274002050') === false) {
						...
						await member.roles.add(roleAdd);
						this.logger.info(`${member.id} (${member.displayName}) наш`);
					}
					if (member.roles.cache.has('918778640869773334') === true) {
						const roleRemove = this.guild.roles.cache.find(
							(role, key, collection) => role.id === '918778640869773334'
						);
	...
	private async minutesInVoice() {
		...
		const allChannels = this.guild.channels.cache.toJSON();
		...
		_.forEach(channels, (channel: ds.VoiceChannel) => {
			if (channel.members.toJSON().length > 0) {
				_.forEach(channel.members.toJSON(), async (member) => {
					...
					const discordUser = await this.database.findOneByUserID(member.id);
					...
					await this.database.addSecond(member.id);
				});

				if (
					channel.name.startsWith('Кибер') &&
	...
	private async cleaner() {
		...
		this.guild.channels.cache.forEach((channel) => {
			if (channel.type === 'GUILD_VOICE' && channel.name === voiceChannels.name) {
	...
	private async addUsersInVoiceBan() {
		...
		for (let i in warningsID) {
			if (warningsID[i].count > 3 && this.banVoiceUsers.includes(i) === false) {
				this._log(null, `User <@${i}> created too many channels`);
	...
	private async createVoiceChannel() {
		 ...
		 */
		this.client.channels.fetch(voiceChannels.duo).then((channel: ds.VoiceChannel | null) => {
			if (channel == null || channel.full == null || channel.full === false) return;

			const parent = channel.parent;
			const guild = channel.guild;
			const name = voiceChannels.name;
		 ...
		 */
		this.client.channels.fetch(voiceChannels.trio).then((channel: ds.VoiceChannel | null) => {
			if (channel == null || channel.full == null || channel.full === false) return;
		 ...
		 */
		this.client.channels.fetch(voiceChannels.four).then((channel: ds.VoiceChannel | null) => {
			if (channel == null || channel.full == null || channel.full === false) return;
		 ...
		 */
		this.client.channels.fetch(voiceChannels.five).then((channel: ds.VoiceChannel | null) => {
			if (channel == null || channel.full == null || channel.full === false) return;
	...
	private async run() {
		this.client.on('ready', () => {
			this.logger.info('Discord are ready');

			DiscordMusic.init(this.guild);
		});
		...
		this.client.on('messageCreate', async (msg) => {
			...
			if (info.content.length >= 3 && msg.guildId === '437601028662231040') {
				this.database.addMessage(info.authorID);
			}
			...
			if (
				...
				if (info.command === 'db_add') {
					...
					await this.database.addMessage(userID, amount);
					const user = await this.database.findOneByUserID(userID);
					info.channel.send({
			...
			if (info.isGuild === true && msg.guild.id === '437601028662231040') {
				if (info.command === 'me') {
					const user = await this.database.findOneByUserID(info.authorID);
					if (!user) {
						this.logger.error(
						...
						const embed = new ds.MessageEmbed()
							...
							.setDescription(
								`Сообщений: ${
									user.messages
								}\nВремя в голосовых каналах: ${Tools.toDDHHMMSS(
									`${user.minutesInVoice}`
								)}`
							)
							.setFooter(`With ❤️ by NidhoggBot v2.0`);
					...
					if (info.command === 'play' || info.command === 'p') {
						...
						if (result.error) {
							await info.channel
								...
								.then((msg) => this._delete(msg, 5000));
						} else {
							await info.channel
								.send({
